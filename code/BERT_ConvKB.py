...
def load_data(args):
    ...
    corpus_no_sim = Corpus(args, train_data, validation_data, test_data, entity2id, relation2id, args.batch_size_gat, args.valid_invalid_ratio_gat, unique_entities_train, args.get_2hop)
    ...
    return corpus, torch.FloatTensor(entity_embeddings), torch.FloatTensor(relation_embeddings), corpus_no_sim, bert_model, train_network, train_data, test_data, all_e1_to_multi_e2

# 得到语料库，实体嵌入矩阵，关系嵌入矩阵
_, entity_embeddings, relation_embeddings, Corpus_, bert_model, train_network, train_data, test_data, all_e1_to_multi_e2 = load_data(args)
...
def train_conv(args):
    ...
    model_conv = SpKBGATConvOnly(bert_model, entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim,
    ...
    model_conv.final_entity_embeddings = nn.Parameter(entity_embeddings.cuda())
    model_conv.final_relation_embeddings = nn.Parameter(relation_embeddings.cuda())
    ...
    for epoch in range(args.epochs_conv):
        ...
        if (epoch + 1) % 20 == 0:
            save_model(model_conv, args.data, epoch,
                       args.output_folder + "ConvKB_BERT/")
...
def evaluate_conv(args, unique_entities, train_network):
    ...
    model_conv.load_state_dict(torch.load(
        '{0}ConvKB_BERT/trained_{1}.pth'.format(args.output_folder, args.epochs_conv - 1)), strict=False)
